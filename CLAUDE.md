...
- **A segment's MRCA has no branch above it.** Segments are dropped from a lineage once they coalesce, so the branch above a segment's last coalescence — and the root branch, above everything — carries an annotation that omits it. The branch above a node and the vertical bar below it are therefore decided separately: without that, a segment's two final lineages run to the same time and stop, appearing never to meet.
