...
def _expand_segments(segments):
    """Convert a list of (start, end, ...) range tuples into the sorted,
    deduplicated list of integer segment indices they cover.
    e.g. [(0.0, 3.0, 3)] -> [0, 1, 2]
         [(1.0, 2.0, 2)] -> [1]
         [(0.0, 1.0, 2), (2.0, 3.0, 2)] -> [0, 2]
    """
    indices = set()
    for v in segments:
        segs = [int(s) for s in range(int(v[0]), int(v[1]))]
        indices.update(segs)
...
def main(argv=None):
    ...
    for rep, (ntime, nsample, edges, _events, _graph) in enumerate(sims):
        ...
        if args.output == "-":
            ...
            # Print nodes table
            sys.stdout.write(f"\n--- graph.nodes (rep {rep}) ---\n")
            sys.stdout.write(f"{'id':>4} {'time':>12} {'type':<14} {'meta'}\n")
            for n in _graph.nodes:
                sys.stdout.write(
                    f"{n.id:>4} {n.time:>12.6f} {n.type:<14} {n.meta if n.meta else ''}\n")

            # Print edges table
            sys.stdout.write(f"\n--- graph.edges (rep {rep}) ---\n")
            sys.stdout.write(f"{'parent':>6} {'child':>6} {'segments'}\n")
            for e in _graph.edges:
                sys.stdout.write(f"{e.parent:>6} {e.child:>6} {e.segments}\n")
            sys.stdout.write("\n")
            ...
            # Write nodes table as CSV alongside the tree sequence file
            nodes_path = f"{path}.nodes.csv"
            with open(nodes_path, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["id", "time", "type", "meta"])
                for n in _graph.nodes:
                    writer.writerow([n.id, n.time, n.type, n.meta])
            sys.stdout.write(f"wrote {nodes_path}\n")

            # Write edges table as CSV alongside the tree sequence file
            edges_path = f"{path}.edges.csv"
            with open(edges_path, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["parent", "child", "segments"])
                for e in _graph.edges:
                    # Segments recorded as integer indices (0, 1, 2, ...)
                    # instead of [start, end) range tuples.
                    writer.writerow([e.parent, e.child, _expand_segments(e.segments)])
            sys.stdout.write(f"wrote {edges_path}\n")
